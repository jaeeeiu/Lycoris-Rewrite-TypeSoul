# Combat Tab
1. Tags are not fully implemented for every timing meaning filtering for moves are broken. Don't use it. It will be fixed in a later update.

+ Auto Defense
  - Enable Notifications
  - Enable Visualizations
  - Dash On Parry Cooldown
  - Deflect Block Fallback
  - Allow Failure
    - Failure Rate
    - Dash Instead Of Parry Rate
    - Fake Mistime Rate
  - Auto Defense Filters (see 1.)
    - Filter Out M1s
    - Filter Out Mantras
...
+ ESP Customization
  ...
  - ESP Fonts (Roblox fonts, to be specific)
+ ESP Optimizations
  - ESP Split Updates (split frames 1f - 64f)
  - No Persistent ESP
+ World Visuals
  - Modify Field Of View
  - Modify Ambience (see 1.)
  - Original Ambience Color
+ Player ESP (will show grade)
  - Enable + Keybind
  - Show Distance
  - Show Health In Bars (+ percentage)
  - Show Ultimate Percentage
  - Show Race
  - Show Combat Timer
  - Show Element
  - Mark Allies
  - Player Name Type (Character Name, Player Name, Display Name)
+ (Mission/Bounty) Board ESP
  - Enable + Keybind
  - Show Distance
+ (NPC/Mob) ESP
  - Enable + Keybind
...
# Exploits Tab
1. (caveat) This was ported over from Deepwoken. They're generic and just there to be there when you can use that option. They will not work on mostly anything. Specialized stuff will come later.

2. (caveat) This feature puts your body very high in the sky with your character staying at the same spot. Putting you into the ground will have to researched later.

3. (caveat) You cannot see the UI at the Main Menu currently. So, if you want to teleport from there, you must go into a slot first.

+ Teleport
  - Teleport To (Hell Ring 1/2/3 + Soul Society + Library + KT + Fake KT + Rukon + Hueco + Maze + Las Noches + Wandenreich + Matchmaking + AFK World) (see 3.)
  - Teleport Slot
+ Mob Exploits
  - Void Mobs (see 1.)
+ Local Character Exploits
  - Pathfind Breaker (see 1.)
  - Invisibility (see 2.)
+ Game Exploits
  - Book Fragments Exploit (only visible when in Library)
